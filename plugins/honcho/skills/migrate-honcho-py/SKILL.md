...
description: Migrate Honcho Python SDK code from v1.6.0 to v2.1.1 from Codex. Use when upgrading the `honcho` package, fixing breaking changes after upgrade, or when errors mention AsyncHoncho, observations, Representation, `.core`, get_config, or self-hosted base URL/auth-token migration.
---
...
Use `base_url` or `HONCHO_URL` for self-hosted API servers. If old code used `HONCHO_AUTH_TOKEN`, keep supporting it by mapping it to the SDK's `api_key` parameter:
...
client = Honcho(
    ...
    base_url=os.getenv("HONCHO_URL"),  # e.g. http://localhost:8000
)
```

For self-hosted deployments with `AUTH_USE_AUTH=false`, `api_key` may be omitted. For `AUTH_USE_AUTH=true`, pass an admin or scoped JWT token.
