...
- [ ] Replace legacy `HONCHO_AUTH_TOKEN`-only setup with `HONCHO_API_KEY` support, or map `HONCHO_AUTH_TOKEN` into `api_key`
- [ ] Use `base_url=` or `HONCHO_URL` for self-hosted API servers
- [ ] Omit `api_key` only when the self-hosted server has `AUTH_USE_AUTH=false`
