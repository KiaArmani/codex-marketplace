...
description: Integrate Honcho memory and social cognition into existing Python or TypeScript codebases from Codex. Use when adding the Honcho SDK, setting up peers, configuring sessions, implementing dialectic chat for AI agents, wiring Honcho into bot frameworks, or targeting hosted or self-hosted Honcho with API key or auth-token authentication.
---
...
After exploring the codebase, clarify integration requirements before implementing. In Codex Plan mode, use `request_user_input` if available. Otherwise ask concise direct questions in chat. Adapt based on what you learned in Phase 1:

#### Question Set 1 - Entities & Peers

Ask about which entities should be Honcho peers:

- "Which entities should Honcho track and build representations for?"
- Offer options based on what you found, such as end users only, users plus AI assistant, users plus multiple AI agents, or all participants including third-party services.
...
- "How should your AI access Honcho's user context?"
- Options:
...
- "How should conversations map to Honcho sessions?"
- Offer options based on their app, such as one session per chat thread, one session per user, multi-user group sessions, or custom session logic.

#### Question Set 4 - Specific Queries (if using pre-fetch pattern)

If they chose pre-fetch, ask what context matters:

- "What user context should be fetched for the AI?"
...
- Confirm environment variables for API key/auth token and base URL are documented
...
2. **Configure authentication**:
   - Hosted Honcho: ask the user to get an API key from <https://app.honcho.dev> and set `HONCHO_API_KEY`.
   - Self-hosted Honcho with `AUTH_USE_AUTH=true`: use the admin or scoped JWT as `HONCHO_AUTH_TOKEN`, or set it directly as `HONCHO_API_KEY`.
   - Self-hosted Honcho with `AUTH_USE_AUTH=false`: no API token is required for SDK calls, but still set the API base URL.

3. **Set the API URL when self-hosted**:

   ```bash
   export HONCHO_URL=http://localhost:8000       # Python/TypeScript SDK
   export HONCHO_BASE_URL=http://localhost:8000  # honcho CLI
   ```

   `HONCHO_MCP_URL` is different: it points at the MCP server/Worker, not the raw Honcho API.

4. **Verify with the CLI** (optional but recommended). If the user has the Honcho CLI installed (`uv tool install honcho-cli`), they can validate their setup before writing any integration code:
...
honcho = Honcho(
    workspace_id="my-app",
    api_key=os.getenv("HONCHO_API_KEY") or os.getenv("HONCHO_AUTH_TOKEN"),
    base_url=os.getenv("HONCHO_URL"),
)
...
honcho = Honcho(
    ...
    api_key=os.getenv("HONCHO_API_KEY") or os.getenv("HONCHO_AUTH_TOKEN"),
    base_url=os.getenv("HONCHO_URL")  # unset for hosted default
)
...
const honcho = new Honcho({
    ...
    apiKey: process.env.HONCHO_API_KEY ?? process.env.HONCHO_AUTH_TOKEN,
    baseURL: process.env.HONCHO_URL // unset for hosted default
});
```
...
- [ ] Set up `HONCHO_API_KEY` or `HONCHO_AUTH_TOKEN` environment variable
- [ ] For self-hosted deployments, set `HONCHO_URL` for SDKs and `HONCHO_BASE_URL` for the CLI
