---
name: honcho-cli
description: Inspect and debug hosted or self-hosted Honcho workspaces via the `honcho` CLI from Codex. Use when investigating peer representations, memory state, session context, queue status, dialectic quality, API key/auth-token setup, or any task that requires introspection of a Honcho deployment.
---
...
- **Self-hosted auth**: pass the self-hosted admin/scoped JWT as `HONCHO_API_KEY` or `--api-key`. If the token is stored as `HONCHO_AUTH_TOKEN`, export `HONCHO_API_KEY="$HONCHO_AUTH_TOKEN"` before running CLI commands. Point the CLI at the self-hosted API with `HONCHO_BASE_URL` or `honcho init --base-url`.
...
### Self-hosted connection check

```bash
HONCHO_API_KEY="$HONCHO_AUTH_TOKEN" \
HONCHO_BASE_URL=http://localhost:8000 \
honcho doctor --json
```

Use `HONCHO_BASE_URL=https://api.honcho.dev` for the hosted service. Use the raw Honcho API URL here, not the MCP Worker URL.
