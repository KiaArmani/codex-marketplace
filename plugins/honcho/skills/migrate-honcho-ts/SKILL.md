...
description: Migrate Honcho TypeScript SDK code from v1.6.0 to v2.1.1 from Codex. Use when upgrading @honcho-ai/sdk, fixing breaking changes after upgrade, or when errors mention removed APIs like .core, getConfig, observations, snake_case properties, or self-hosted base URL/auth-token migration.
---
...
## Self-hosted Auth and Base URLs

Use `baseURL` or `HONCHO_URL` for self-hosted API servers. Note the capital `URL`; `baseUrl` is rejected by strict validation. If old code used `HONCHO_AUTH_TOKEN`, keep supporting it by mapping it to `apiKey`:
...
const honcho = new Honcho({
  ...
  baseURL: process.env.HONCHO_URL // e.g. http://localhost:8000
})
```

For self-hosted deployments with auth disabled, `apiKey` may be omitted. For auth enabled, pass an admin or scoped JWT token.
