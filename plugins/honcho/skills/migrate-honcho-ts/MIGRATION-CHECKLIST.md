...
- [ ] Replace legacy `HONCHO_AUTH_TOKEN`-only setup with `HONCHO_API_KEY` support, or map `HONCHO_AUTH_TOKEN` into `apiKey`
- [ ] Use `baseURL` or `HONCHO_URL` for self-hosted API servers
- [ ] Fix any `baseUrl` typo to `baseURL`
- [ ] Omit `apiKey` only when the self-hosted server has `AUTH_USE_AUTH=false`
